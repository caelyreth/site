...
export const entries: EntryGenerator = constellation_page_entries
...
export const load: PageServerLoad = async ({ depends, params }) => {
  ...
  const { constellations: entries, document: index } =
    await constellation_page(page)
  if (!entries) throw error(404, '未找到星群页')
  return { constellations: entries, index }
}

...
export type PulseTimeline = Readonly<{
  ...
  foreground_contract_end: number
  foreground_contract_start: number
  foreground_return_end: number
  foreground_return_start: number
...
export function create_pulse_timeline(
  ...
  const foreground_contract_start = FOREGROUND_CONTRACT_DELAY
  ...
  const foreground_contract_end =
    foreground_contract_start + contract_duration
  const foreground_return_start = camera_apex_time + FOREGROUND_RETURN_LAG
  const foreground_return_end =
...
export function roller_motion_duration(camera_duration: number) {
  return Math.min(
    ROLLER_MAX_DURATION,

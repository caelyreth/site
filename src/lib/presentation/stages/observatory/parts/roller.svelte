<script lang="ts">
  ...
  type IndexRollerProps = {
    active: boolean
    motion: SkyMapRouteMotionStatus
...
<section
  ...
  class="aperture-strip roller-strip"
...
>
  ...
  <span class="aperture-strip__meta roller-meta">AP-04</span>
  <span class="aperture-strip__readout roller-readout">
...
<style>
  .roller-strip {
    ...
    --strip-frame-z-index: 3;
    --strip-label-z-index: 4;
    --strip-reveal-delay: 360ms;
    --strip-active-line-mix: 56%;
  ...
  .material-exposure {
    ...
    transition:
      opacity var(--dur-travel) var(--ease-out),
  ...
  .continuity-line {
    ...
    transition:
      background-color var(--dur-surface) var(--ease-out),
      opacity var(--dur-surface) var(--ease-out);
  }
  ...
  .continuity-line::after {
    ...
    transition:
      border-color var(--dur-surface) var(--ease-out),
      transform var(--dur-travel) var(--ease-out);
  }
  ...
  .roller-strip.active .continuity-line {
    background: color-mix(in oklab, var(--signal) 64%, var(--strip-line));
  ...
  .roller-strip.active .continuity-line::before {
    animation: signal-traverse 1.28s var(--ease-traverse)
  ...
  @media (max-width: 38rem) {
    .roller-strip {
      --strip-frame-inset: 0.4rem 0.2rem;
    }
